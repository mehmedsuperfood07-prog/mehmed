...
- A fixed **library of section types** is built once, matching the sections actually used across the template: `Hero`, `LogoCarousel`, `PageHeader`, `RichText`, `ImageWithText`, `StatsCounter`, `FeatureGrid`, `ProductGrid`, `ProductList`, `CoverageArea`, `TestimonialSlider`, `CTABanner`, `FAQAccordion`, `Gallery`, `ContactForm`, `ContactInfo`, `QuoteForm`. Three rounds of gaps were only found by the client directly comparing the built site against the template page by page, not by the original audit here: `LogoCarousel` (a "trusted by" logo band below the Hero), and — from a full pass over About/Menu/Contact — `PageHeader` (every inner page uses a completely different split hero than the homepage's `Hero`), `ContactInfo` (a divided-row Head-Office/Phone/Email/Hours list, missing entirely), and `ProductList` (the Menu page groups products by category with an image beside a row-list, not `ProductGrid`'s flat grid, which is actually the homepage-only pattern). Treat section 5 as a checklist to keep re-verifying against — not a one-time pass — and when told "check the rest of the pages," actually visit and screenshot every template page rather than reasoning from the first pass's notes.
...
- **Every full-bleed primary-green "band" section is a fully-rounded card** (`rounded-[2.5rem]` on all four corners, `overflow-hidden` so the background actually clips to it), not just a plain rectangle — `LogoCarousel`, `ProductGrid`, `FeatureGrid`'s dark variant, and `ContactForm` all get this. This echoes Hero/PageHeader's bottom-only rounding (`rounded-b-[2.5rem]`), so a colored band sitting right after either one reads as a matching, continuous rounded shape rather than a rounded edge running into a hard square corner — this was a direct client correction, not something the original audit caught. `CTABanner` already had this. Remember `overflow-hidden` is required alongside the radius — a rounded corner with default `overflow: visible` doesn't actually clip the section's own background color, so the rounding is invisible without it.
- **Logo carousel**: a short full-bleed primary-green band directly below the Hero, an infinitely-scrolling ticker (pure CSS `@keyframes` marquee, not Embla — the list renders twice back to back and scrolls exactly 50% for a seamless loop) of "trusted by" logos. Content is generic business-category badges ("General Stores," "Bakeries," etc.), not fabricated specific client names — Mehmed's actual customers are small shops without notable logos, and inventing named partners would be a real content-integrity problem, not just a placeholder. Swap in real partner/distributor logos via each item's `image_url` once they exist.
- **Inner-page header (`PageHeader`)**: About, Products, Contact, FAQ, Reviews, and Become a Distributor all open with this, NOT `Hero` — a completely different, simpler pattern than the homepage: a full-bleed two-column split, photo+large title on one side (same header-floats-transparently-on-top treatment as Hero, including the mobile stacking), a solid cream info panel with a smaller heading + short body on the other. No floating widget cards, no CTA buttons. Conflating this with `Hero` (reusing one component for both) was the single biggest fidelity gap found when auditing the rest of the site.
- **Product/menu sections are two different things**, not one: `ProductGrid` is the homepage's "Best Sellers" pattern (full-bleed primary-green band, white rounded card floating on top holding a flat grid) — correct as originally built. `ProductList` is the actual Products/Menu page's pattern: products grouped by category, each group a photo beside a stack of lime row-cards (name + pack size in accent font on one line, description below) — a flat grid was wrong for this page specifically.
- **Contact info list (`ContactInfo`)**: a divided-row block (Head Office / Phone / WhatsApp / Email / Opening Hours, label left, value right in the italic accent font, hairline rule between rows) that reads live from `site_settings` rather than duplicating that data into section content — same single-source-of-truth approach as `Footer`. This section didn't exist at all in the first pass; the Contact page had no info display beyond the footer.
...
- **Forms are two different styles, not one shared style**: `QuoteForm` clones the template's light two-column "Book Us" layout (cream-filled inputs, dark info card with a small circular badge overlapping its top-right corner). `ContactForm` clones the Contact page's actual form, which is a full-bleed dark-primary band with underlined (not filled) inputs in a 3-column row (name/email/phone) above the message field, and a lime submit button — this was wrong (light, single-column) in the first pass.
...
- **Deliberately deferred template patterns** (found during the page-by-page audit, not built): the About page's "Our Story" has a 4-card timeline with a large number per card (e.g. a founding year) — not built as a distinct pattern; close enough is achievable via the existing `feature_grid` using the `icon` field for a number if wanted later. "Our Philosophy" uses a stacked photo-on-top / centered-text-below variant of the image+text pattern, distinct from `ImageWithText`'s side-by-side layout — not built as a separate variant. The "RollingBites Crew" team/staff section was skipped entirely as a deliberate scope call: individual staff headshots suit a food-truck personality brand more than a B2B distributor, and Mehmed has no team photos to use anyway. None of these three are bugs — they're scope decisions, recorded here so they don't get "rediscovered" as unexplained gaps.
