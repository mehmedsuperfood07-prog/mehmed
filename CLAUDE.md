...
- `/components/sections/*` — the closed section library (PLAN.md §3.1): `types.ts` (content shape + `FieldSchema` per type), `schemas.ts` (admin field schema + default content per type), `registry.tsx` (type → public renderer, used by `PageSections.tsx`), then one renderer file per type (`Hero.tsx`, `RichText.tsx`, `ProductGrid.tsx`, `TestimonialSlider.tsx` + its client-side `TestimonialCarousel.tsx`, `ContactForm.tsx`, `QuoteForm.tsx` + `QuoteFormFields.tsx`, etc.). `Reveal.tsx` is the shared scroll-reveal wrapper nearly every renderer uses. `Button.tsx` is the one pill-button component (solid/lime/outline variants) — use it instead of hand-rolling button markup. `Accent.tsx` exports `renderHeading(text, color)`, which every heading passes through to support the `**phrase**` italic-accent syntax (PLAN.md §5.2) — see the "heading accent syntax" convention above before touching any heading.
...
- **Colors are theme tokens, not literals.** The palette in PLAN.md §5.1 is confirmed from packaging, but applied the template's way: white page background, exactly two colors doing real work (`primary` green for full-bleed sections/buttons, `lime` for card backgrounds/accent text), everything else (`maroon`, `red`, `gold`) held in reserve rather than forced into the current sections. Every color goes through Tailwind theme tokens (`bg-primary`, `bg-lime`, `text-ink`, etc.) defined in `app/globals.css`, never a hardcoded hex in a component.
- **Design fidelity to the template is a hard requirement, not a vibe.** The client explicitly asked for an exact clone of `templatekit.selaraswp.com/restaurant`, and the first implementation pass drifted into a generic look instead — real rework was needed to fix it (PLAN.md §5 has the full before/after). Before inventing a new visual pattern for a section, check whether the template already has one and clone its layout/spacing/typography, not just its content structure. When in doubt, re-audit the live template (screenshot + `getComputedStyle` via the browser tools) rather than guessing from memory.
- **Heading accent syntax:** any heading field supports `**phrase**` for the italic-serif emphasis treatment (`components/sections/Accent.tsx`'s `renderHeading`). Every call site must pass an explicit accent `color` matching its own background (`text-lime` on dark/photo sections, the default `text-primary` on white ones) — there is no safe default, and getting this wrong makes the accented text invisible. This exact bug shipped once during the rebuild and was only caught by looking at a real screenshot, not by the type checker.
- **Check mobile, not just desktop, before calling a section done.** Layouts built around `position: absolute` (the Hero's floating widget cards and bottom-anchored headline/CTA block) can look perfect on desktop and completely collide on a narrow viewport — this happened once during the rebuild. Verify with the browser's mobile preset, not just a visual skim at desktop width.
...
- Next.js 16 (App Router) + TypeScript + Tailwind v4 scaffolded, brand palette wired into `app/globals.css` as theme tokens
- **Design was rebuilt once already to actually match the template** (Inter + Instrument Serif italic accent, pill buttons, white base with two-color discipline, template-cloned section layouts — PLAN.md §5 has the full detail and the reasoning). The first version used a different font (Fraunces) and a generic Tailwind-defaults look instead of cloning the template; don't reintroduce that drift by "improving" a section's look without checking what the template actually does there.
...
- **All 17 section-library types have real renderers** (`components/sections/`): Hero (homepage only), PageHeader (every inner page's actual hero — see below), LogoCarousel (pure-CSS marquee), RichText, ImageWithText, StatsCounter (count-up on scroll), FeatureGrid, ProductGrid (homepage grid, live DB query), ProductList (Menu-page category-grouped listing, live DB query), CoverageArea, TestimonialSlider (live DB query + Embla carousel), CTABanner, FAQAccordion, Gallery, ContactForm (dark full-bleed variant), ContactInfo (reads `site_settings` live), QuoteForm — the last two forms fully wired to `submitLead`, not just visual.
- **`Hero` vs `PageHeader` — don't conflate these.** `Hero` is the homepage-only rich pattern (floating widget cards, full overlay). `PageHeader` is what every other page actually opens with in the template (split photo+title / cream info panel, no floating cards, no CTAs). Reusing `Hero` everywhere was the single biggest fidelity gap found when the client asked to "check the rest of the pages" — don't repeat that mistake by defaulting a new page's first section to `Hero`; check which one the template actually uses for that page type first.
- **`ProductGrid` vs `ProductList` — also not interchangeable.** `ProductGrid` is the homepage's flat "Best Sellers" grid inside a white card on a green band. `ProductList` is the Products/Menu page's category-grouped listing (photo beside a stack of lime row-cards). Picking the wrong one for a given page is exactly the kind of mistake the audit above was trying to prevent.
...
- **Not built yet**: Blog (Tiptap editor + public listing/detail), Gallery page (see above), a nav editor (header_nav/footer_nav are schema-only).
- **SEO/technical audit done** (pre-indexing pass): `lib/site.ts` holds the canonical `SITE_URL` constant (`https://mehmedsuperfood.pk` — hardcoded, not an env var, since canonical/OG URLs must point to the same authority regardless of which environment renders them). `lib/pages.ts`'s `pageMetadata` now always emits a **self-referencing canonical** (`page.canonical_url` remains a rare override, not the only path to a canonical tag) plus full OpenGraph/Twitter tags, falling back to `site_settings.logo_url` as the default share image. `app/robots.ts` and `app/sitemap.ts` are real routes now (sitemap pulls from `pages` where `status=published` and `no_index=false`); robots disallows `/admin`, and `app/admin/layout.tsx` additionally sets `noindex` on the whole admin tree as defense in depth (proxy.ts auth-gates the dashboard, but `/admin/login` itself was previously a publicly indexable page). `getPageBySlug` now filters on `status=published` — previously a **draft page was publicly reachable at its slug the moment it was created** (pages default to `draft`), fixed here. `components/seo/OrganizationSchema.tsx` renders sitewide Organization JSON-LD (name/logo/address/contactPoint/sameAs) from live `site_settings`, included in `app/(site)/layout.tsx`.
  - **Still open before submitting to Search Console**: `mehmedsuperfood.pk`'s DNS still isn't pointed at Vercel (see "Nameservers" — unresolved as of this writing), so canonical URLs currently resolve to nothing publicly; don't request indexing until that's fixed. `blog_posts` has `seo_title`/`seo_description` but no `canonical_url`/`no_index`/`og_image_url` columns (fine for now since no blog routes exist yet — revisit when Blog is actually built). `products` has no SEO columns and no individual detail routes at all (`/products` is listing-only) — not currently a gap since there's no indexable per-product URL to be missing metadata on.
  - **Real favicon wired**: the client sent a proper square icon (96×96 PNG, badge-style "MEHMED" mark) — it's `app/icon.png` + `app/apple-icon.png` (Next.js's file-convention icons, auto-generates the `<link rel="icon">`/`<link rel="apple-touch-icon">` tags, no code needed). The old scaffold `app/favicon.ico` was deleted rather than kept alongside it, to avoid two competing icon sources. 96×96 covers standard browser favicon sizes fine; if the client ever sends a higher-res version (180×180+ is ideal for `apple-icon.png`), swap the files in place.
