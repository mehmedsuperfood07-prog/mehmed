...
- **Admin dashboard**, verified as far as possible without the admin password (build, lint, real-browser auth-redirect checks on every route, and direct DB queries to confirm joins/data shapes) — actual sign-in and clicking through the forms still needs the client to test:
  ...
  - Image fields everywhere (`SectionEditorFields`, `ProductForm`, `TestimonialForm`) use the same `ImageUploadField` → `uploadImage` server action → Supabase Storage `media` bucket
  - Sidebar nav (`app/admin/(dashboard)/layout.tsx`) lists Overview/Pages/Products/Testimonials/Leads — extend `NAV_ITEMS` as Blog/Settings screens get built; don't add dead links ahead of the page existing
...
- **Not built yet**: Blog (Tiptap editor + public listing/detail), Settings screen (site_settings — contact info, nav, is currently only editable via the seed script or raw SQL), Resend email notifications, and the remaining pages from PLAN.md §2 (Gallery, Reviews, FAQ, Become-a-Distributor).
