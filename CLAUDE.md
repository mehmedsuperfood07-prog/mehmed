...
- **All 14 section-library types have real renderers** (`components/sections/`): Hero, LogoCarousel (pure-CSS marquee, added late after being missed in the original template audit — see PLAN.md §3.1), RichText, ImageWithText, StatsCounter (count-up on scroll), FeatureGrid, ProductGrid (live DB query), CoverageArea, TestimonialSlider (live DB query + Embla carousel), CTABanner, FAQAccordion, Gallery, ContactForm, QuoteForm — the last two fully wired to `submitLead`, not just visual.
